...
def main():
    ...
    log_T = hydraulic_oil_data['log (T) '].to_numpy()
    delta_log_T = hydraulic_oil_data['Δlog (T) '].to_numpy()
    log_log_v_plus_0_8 = hydraulic_oil_data['log {log (v + 0.8)}'].to_numpy()
    delta_log_log_v_plus_0_8 = hydraulic_oil_data['Δlog {log (v + 0.8)}'].to_numpy()
    ...
    plt.title('hydraulic Oil: Least-squares graph of log{log(v + 0.8)} against log(T)')
